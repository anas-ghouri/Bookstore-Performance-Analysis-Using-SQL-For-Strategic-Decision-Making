...
select Book_name, product_disc_price*qty_sold as Revenue_after_discount
from books_data
...
-- (average rating ≥ 4.5 and > 1000 copies sold)?
...
from books_data
group by author
having Avg_rating>=4.5 and Total_copies_sold>1000
...
select Distinct Book_name, product_rating, qty_sold
from books_data
...
SELECT 
    Book_name, author, product_rating, product_disc_price, qty_sold
FROM books_data
WHERE product_rating >= 4.5 
    AND product_disc_price BETWEEN 300 AND 600 
    AND qty_sold BETWEEN 20 AND 100
